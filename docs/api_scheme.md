...
| POST | `/api/v1/auth/register/` | нет | Регистрация пользователя с отправкой кода подтверждения email |
| POST | `/api/v1/auth/verify-email/` | нет | Подтверждение email по 6-значному коду |
| POST | `/api/v1/auth/verify-email/resend/` | нет | Повторная отправка кода подтверждения email |
| POST | `/api/v1/auth/token/` | нет | Получение DRF-токена по `email/password` только после подтверждения почты |
