...
- Локальные тесты и единый сценарий приемочной проверки могут использовать SQLite, кроме сценариев графовой проекции.
...
- Синхронизация ML, graph и faculty-сервисов выполняется через outbox API, снимок состояния, подтверждения обработки, контрольные точки и повторное чтение.
...
- Межсервисный доступ к методам outbox выполняется через служебные токены доступа.
