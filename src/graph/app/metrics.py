...
HTTP_REQUESTS = Counter(
    "dsa_graph_http_requests_total",
    "Total HTTP requests handled by the graph service.",
    ("method", "path", "status"),
    registry=REGISTRY,
)
HTTP_REQUEST_DURATION = Histogram(
    "dsa_graph_http_request_duration_seconds",
    "HTTP request duration in seconds for the graph service.",
    ("method", "path", "status"),
    registry=REGISTRY,
)
READINESS_STATUS = Gauge(
    "dsa_graph_readiness_status",
    "Graph readiness status by dependency check; 1 means healthy, 0 means degraded.",
    ("check",),
    registry=REGISTRY,
)
SYNC_REQUESTS = Counter(
    "dsa_graph_outbox_sync_requests_total",
    "Outbox sync attempts by mode and result for the graph service.",
    ("mode", "result"),
    registry=REGISTRY,
)
SYNC_EVENTS = Counter(
    "dsa_graph_outbox_sync_events_total",
    "Outbox events processed by the graph service.",
    ("mode",),
    registry=REGISTRY,
)
POLLER_CYCLES = Counter(
    "dsa_graph_poller_cycles_total",
    "Background poller cycles by result for the graph service.",
    ("result",),
    registry=REGISTRY,
)
GRAPH_NODES = Gauge(
    "dsa_graph_nodes",
    "Graph node count by type.",
    ("type",),
    registry=REGISTRY,
)
GRAPH_EDGES = Gauge(
...
LAST_ACKED_EVENT_ID = Gauge(
    "dsa_graph_outbox_last_acked_event_id",
    "Last outbox event id mirrored by the graph service.",
    registry=REGISTRY,
)
...
def update_state_metrics(summary: dict) -> None:
    ...
    for node_type, count in nodes.items():
        GRAPH_NODES.labels(type=str(node_type)).set(int(count or 0))
    GRAPH_EDGES.set(int(summary.get("edges") or 0))
