...
def test_moderation_queue_shows_pending_projects():
    ...
    cpprp = _make_cpprp()
    customer = _make_customer()
    _make_project(owner=customer, status=ProjectStatus.ON_MODERATION, title="Pending XYZ")
    _make_project(owner=customer, status=ProjectStatus.DRAFT, title="Draft ABC")
    client = Client()
    client.force_login(cpprp)
    response = client.get(reverse("frontend:moderation_list"))
    assert response.status_code == 200
    ...
    assert "Pending XYZ" in content
    # Draft project must NOT appear in the moderation queue
    assert "Draft ABC" not in content
