...
def _project_text(project: Project) -> str:
    return " ".join(
        [
            ...
            getattr(project, "vacancy_title", "") or "",
            getattr(project, "thesis_title", "") or "",
            getattr(project, "implementation_features", "") or "",
            getattr(project, "selection_criteria", "") or "",
...
def _published_projects() -> list[Project]:
    return list(
        Project.objects.filter(status__in=ProjectStatus.catalog_values()).select_related("owner")
    )
...
def _project_payload(project: Project) -> dict[str, object]:
    return {
        ...
        "supervisor_name": getattr(project, "supervisor_name", "") or "",
        "source_type": project.source_type,
    }


def _ml_service_url() -> str:
    """Return ML service base URL.

    Env var takes priority so it can be overridden at runtime (Docker, tests);
    Django settings provide a convenient dev-time default.
    """
